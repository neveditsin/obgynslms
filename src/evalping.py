...
def build_performance_table(
    ...
    *,
    ...
    return perf
# ----------------------------
# Adjusted function for comparing a reference (e.g., ping_base) vs others in its group
# ----------------------------
def evaluate_target_vs_others(
    dfs: Dict[str, pd.DataFrame],
    target: str,
    *,
    metric: str | Callable = "f1",
    ...
    """
    Evaluates performance and significance for a target method vs other methods in the same document type.
    - Computes full performance table for all models/methods.
    - Computes bootstrap p-values only for matching (target_method, model) vs (other_method, model) pairs within the same doc type.
    - Doc type inferred from target: if '_struct' in target, compare to other '_struct' methods; else compare to non-'_struct' methods.
    - Returns perf (as before), and pair_pvals DataFrame with columns: target_score, other_score, delta, p_value
      indexed by (pair, model).
    - Assumes method names match exactly as in the data (e.g., 'ping_base' vs 'all_models' or 'rand_base').
    ...
    metric_fn = get_metric_fn(metric, average=average, labels=labels) if isinstance(metric, str) else metric
    # Collect unique models and methods
    models = set(model for method, model in systems)
    all_methods = set(method for method, model in systems)
    # Infer doc type and get others
    is_struct = '_struct' in target
    if is_struct:
        others = [m for m in all_methods if '_struct' in m and m != target]
    else:
        others = [m for m in all_methods if '_struct' not in m and m != target]
    rows = []
    total_pairs = len(others) * len(models)
    progress = tqdm(total=total_pairs, desc="Bootstrap target-other pairs")
    for other_meth in others:
        for model in models:
            k_target = (target, model)
            k_other = (other_meth, model)
            if k_target not in systems or k_other not in systems:
                progress.update(1)
                continue
            a = systems[k_target]  # target
            b = systems[k_other]  # other
            ...
            target_score = float(metric_fn(y_true_arr, y_pred_a_arr))
            other_score = float(metric_fn(y_true_arr, y_pred_b_arr))
            rows.append({
                'pair': f'{target} vs {other_meth}',
                'model': model,
    ...
    if not pair_pvals.empty:
        pair_pvals = pair_pvals.set_index(['pair', 'model']).sort_index()
